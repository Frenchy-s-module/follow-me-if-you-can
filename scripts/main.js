...
class FollowMeIfYouCan {
    ...
    static registerSceneControls(controls) {
        if (!(game.user.isGM || game.settings.get("follow-me-if-you-can", "playerAccess"))) return;

        // v12: controls est un tableau; v13: controls est un objet/record avec clés de groupes
        const isArrayStructure = Array.isArray(controls);

        // Récupère ou crée le groupe Token selon la version
        let tokenGroup;
        if (isArrayStructure) {
            tokenGroup = controls.find(c => c.name === "token");
        } else if (controls && typeof controls === "object") {
            // Noms possibles: token / tokens selon systèmes; privilégier "token"
            tokenGroup = controls.token || controls.tokens || null;
        }

        if (!tokenGroup) return;

        // Normalise le tableau d'outils
        tokenGroup.tools = tokenGroup.tools || (isArrayStructure ? [] : {});

        // Fabrique d'outil compatible v12 (array) et v13 (record)
        const addTool = (tool) => {
...
async function showTokenSelectionDialog(followerToken) {
    ...
    // Afficher la boîte de dialogue (sans Application V1/V2)
    ...
    const title = document.createElement('h3');
    title.textContent = game.i18n.localize("FOLLOWMEIFYOUCAN.Dialog.SelectTarget.Title");
    title.style.marginTop = '0';
    const contentWrapper = document.createElement('div');
    contentWrapper.innerHTML = content;
    const buttons = document.createElement('div');
    Object.assign(buttons.style, { display: 'flex', gap: '8px', justifyContent: 'flex-end', marginTop: '12px' });
    const cancelBtn = document.createElement('button');
    cancelBtn.innerHTML = '<i class="fas fa-times"></i> ' + game.i18n.localize("FOLLOWMEIFYOUCAN.Dialog.Cancel");
    const okBtn = document.createElement('button');
    okBtn.innerHTML = '<i class="fas fa-user-plus"></i> ' + game.i18n.localize("FOLLOWMEIFYOUCAN.Dialog.SelectTarget.Follow");
    ...
    okBtn.addEventListener('click', () => {
        const select = contentWrapper.querySelector('#target-token-select');
...
function showStopFollowingDialog() {
    ...
    // Afficher une modale HTML (sans Application V1/V2)
    const overlay = document.createElement('div');
    Object.assign(overlay.style, {
        position: 'fixed',
        inset: '0',
    ...
    const title = document.createElement('h3');
    title.textContent = game.i18n.localize("FOLLOWMEIFYOUCAN.Dialog.StopFollowing.Title");
    title.style.marginTop = '0';

    const form = document.createElement('form');
    const selectAllWrap = document.createElement('div');
    selectAllWrap.className = 'form-group';
    ...
    Object.assign(list.style, { maxHeight: '300px', overflowY: 'auto', marginBottom: '10px' });
    list.className = 'follow-relationships-list';
    list.innerHTML = relationshipsList;
    const stopAllScenesWrap = document.createElement('div');
    stopAllScenesWrap.className = 'form-group';
    stopAllScenesWrap.innerHTML = `
        <label>
    ...
    form.appendChild(selectAllWrap);
    form.appendChild(list);
    form.appendChild(stopAllScenesWrap);
    ...
    stopBtn.type = 'button';
    stopBtn.innerHTML = '<i class="fas fa-user-slash"></i> ' + game.i18n.localize("FOLLOWMEIFYOUCAN.Dialog.StopFollowing.Stop");
    buttons.appendChild(cancelBtn);
    ...
    selectAll.addEventListener('change', (event) => {
        ...
        form.querySelectorAll('input[data-type="follow"]').forEach((el) => {
            el.checked = checked;
        });
    });

    stopBtn.addEventListener('click', () => {
        const stopAllScenes = form.querySelector('#stop-all-scenes');
        if (stopAllScenes && stopAllScenes.checked) {
        ...
        const selectedIds = [];
        form.querySelectorAll('input[data-type="follow"]:checked').forEach((el) => {
            selectedIds.push(el.value);
        });

        if (selectedIds.length > 0) {
            selectedIds.forEach((id) => stopFollowing(id));
